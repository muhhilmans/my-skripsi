<x-modal name="deleteModal{{ $sy->id }}" focusable>
    <div class="p-6">
        <form action="{{ route('school-years.destroy', $sy->id) }}" method="post">
            ...
            <div class="flex justify-between items-center border-b pb-3">
                <h2 class="text-xl font-semibold">{{ __('Hapus Tahun Ajaran') }}</h2>
                <button type="button" x-on:click="$dispatch('close-modal', 'deleteModal{{ $sy->id }}')" class="text-gray-500 hover:text-gray-700">
            ...
            <!-- Modal Body -->
            <div class="mt-4">
                <p>Apakah Anda yakin menghapus tahun ajaran ini?</p>
            ...
            <div class="flex justify-end items-center mt-4 border-t pt-3">
                <x-secondary-button
                    x-on:click="$dispatch('close-modal', 'deleteModal{{ $sy->id }}')">{{ __('Batal') }}</x-secondary-button>
