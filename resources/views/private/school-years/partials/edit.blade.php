<x-modal name="editModal{{ $sy->id }}" focusable>
    <div class="p-6">
        <form action="{{ route('school-years.update', $sy->id) }}" enctype="multipart/form-data" method="post">
            @csrf
            @method('PUT')
            <!-- Modal Header -->
            <div class="flex justify-between items-center border-b pb-3">
                <h2 class="text-xl font-semibold">{{ __('Edit Tahun Ajaran') }}</h2>
                <button x-on:click="$dispatch('close-modal', 'editModal{{ $sy->id }}')"
                ...
                <div class="flex flex-row gap-2">
                    <div class="basis-1/2">
                        ...
                        <x-text-input id="early_year" class="block mt-1 w-full" type="number" name="early_year"
                            :value="old('early_year', $sy->early_year)" required autofocus autocomplete="early_year" />
                    ...
                    <div class="basis-1/2">
                        ...
                        <x-text-input id="final_year" class="block mt-1 w-full" type="number" name="final_year"
                            :value="old('final_year', $sy->final_year)" required autofocus autocomplete="final_year" />
                    ...
                    <div class="flex flex-row gap-2">
                        <div class="basis-1/2">
                            ...
                            <select id="semester" name="semester"
                                ...
                                <option value="1" {{ old('semester', $sy->semester) == '1' ? 'selected' : '' }}>Ganjil</option>
                                <option value="0" {{ old('semester', $sy->semester) == '0' ? 'selected' : '' }}>Genap</option>
                            </select>
                        ...
                        {{-- <div class="basis-1/2">
                            ...
                            <select id="active" name="active"
                                ...
                                <option value="1" {{ old('active', $sy->active) == '1' ? 'selected' : '' }}>Aktif</option>
            ...
            <div class="flex justify-end items-center mt-4 border-t pt-3">
                <x-secondary-button
                    x-on:click="$dispatch('close-modal', 'editModal{{ $sy->id }}')">{{ __('Batal') }}</x-secondary-button>
