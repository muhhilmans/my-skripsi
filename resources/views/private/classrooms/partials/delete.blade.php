<x-modal name="deleteModal{{ $cr->id }}" focusable>
    <div class="p-6">
        <form action="{{ route('classrooms.destroy', $cr->id) }}" method="post">
            @csrf
            @method('DELETE')
            <!-- Modal Header -->
            <div class="flex justify-between items-center border-b pb-3">
                <h2 class="text-xl font-semibold">{{ __('Hapus Tingkat') }}</h2>
                <button type="button" x-on:click="$dispatch('close-modal', 'deleteModal{{ $cr->id }}')" class="text-gray-500 hover:text-gray-700">
            ...
            <!-- Modal Body -->
            <div class="mt-4">
                <p>Apakah Anda yakin menghapus ruang kelas ini?</p>
            ...
            <div class="flex justify-end items-center mt-4 border-t pt-3">
                <x-secondary-button
                    x-on:click="$dispatch('close-modal', 'deleteModal{{ $cr->id }}')">{{ __('Batal') }}</x-secondary-button>
