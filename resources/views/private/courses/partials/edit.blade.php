<x-modal name="editModal{{ $course->id }}" focusable>
    <div class="p-6">
        <form action="{{ route('courses.update', $course->id) }}" enctype="multipart/form-data" method="post">
            ...
            <div class="flex justify-between items-center border-b pb-3">
                <h2 class="text-xl font-semibold">{{ __('Edit Mata Pelajaran') }}</h2>
                <button x-on:click="$dispatch('close-modal', 'editModal{{ $course->id }}')"
            ...
            <div class="mt-4 text-start">
                <div>
                    <x-input-label for="name" :value="__('Mata Pelajaran')" required />
                ...
                <div class="mt-4">
                    <x-input-label for="level_id" :value="__('Kelas')" required />
                    <select id="level_id" name="level_id"
                        ...
                        @foreach ($levels as $level)
                            <option value="{{ $level->id }}" {{ old('level_id', $course->level_id) == $level->id ? 'selected' : '' }}>Paket {{ $level->name }} / Kelas {{ $level->class }}</option>
                        @endforeach
                    </select>
                ...
                <div class="mt-4">
                    <x-input-label for="user_id" :value="__('Tutor')" required />
                    <select id="user_id" name="user_id"
                        class="border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm block w-full p-2.5"
                        required>
                        <option selected disabled>Pilih Tutor</option>
                        @foreach ($tutors as $tutor)
                            <option value="{{ $tutor->id }}" {{ old('user_id', $course->user_id) == $tutor->id ? 'selected' : '' }}>{{ $tutor->name }}</option>
                        @endforeach
                    </select>
                    <x-input-error :messages="$errors->get('class')" class="mt-2" />
                </div>
            ...
            <div class="flex justify-end items-center mt-4 border-t pt-3">
                <x-secondary-button
                    x-on:click="$dispatch('close-modal', 'editModal{{ $course->id }}')">{{ __('Batal') }}</x-secondary-button>
