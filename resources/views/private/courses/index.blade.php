    ...
    <x-slot name="header">
        <div class="flex items-center justify-between">
            ...
            <div class="relative flex items-center">
                ...
                @include('private.courses.partials.create')
            </div>
        </div>
    </x-slot>
                                                    ...
                                                    <tr>
                                                        <td
                                                            ...
                                                            <h4 class="text-gray-700">
                                                                {{ $loop->iteration + $courses->perPage() * ($courses->currentPage() - 1) }}
                                                            </h4>
                                                        </td>
                                                        ...
                                                        <td class="px-4 py-4 text-sm whitespace-nowrap text-center">
                                                            <x-secondary-button x-data=""
                                                                x-on:click.prevent="$dispatch('open-modal', 'editModal{{ $course->id }}')"><i
                                                                    class='bx bx-edit-alt bx-sm'></i></x-secondary-button>
                                                            @include('private.courses.partials.edit')
                                                            <x-danger-button x-data=""
                                                                x-on:click.prevent="$dispatch('open-modal', 'deleteModal{{ $course->id }}')"><i
                                                                    class='bx bx-trash bx-sm'></i></x-danger-button>
                                                            @include('private.courses.partials.delete')
                                                        </td>
                                                    </tr>
                                                @endforeach
                                            @endif
