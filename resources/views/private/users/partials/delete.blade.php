<x-modal name="deleteModal{{ $user->id }}" focusable>
    <div class="p-6">
        <form action="{{ route('users.destroy', $user->id) }}" method="post">
            ...
            <div class="flex justify-between items-center border-b pb-3">
                <h2 class="text-xl font-semibold">{{ __('Hapus User') }}</h2>
                <button type="button" x-on:click="$dispatch('close-modal', 'deleteModal{{ $user->id }}')" class="text-gray-500 hover:text-gray-700">
            ...
            <div class="flex justify-end items-center mt-4 border-t pt-3">
                <x-secondary-button
                    x-on:click="$dispatch('close-modal', 'deleteModal{{ $user->id }}')">{{ __('Batal') }}</x-secondary-button>
