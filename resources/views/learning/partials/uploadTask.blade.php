<x-modal name="uploadTaskModal{{ $task->id }}" focusable>
    <div class="p-6">
        <form action="{{ route('learning.course.uploadTask', ['classroom' => $classroom->id, 'course' => $course->id])}}" enctype="multipart/form-data" method="post">
            @csrf
            {{-- @method('PUT') --}}
            <!-- Modal Header -->
            <div class="flex justify-between items-center border-b pb-3">
                <h2 class="text-xl font-semibold">{{ __('Upload Tugas') }}</h2>
                <button x-on:click="$dispatch('close-modal', 'uploadTaskModal{{ $task->id }}')"
            ...
            <div class="mt-4 text-start">
                ...
                <div>
                    <x-input-label for="file" :value="__('Upload File')" required />
                    {{-- @if ($task->file_path)
                        <div class="mb-2">
            ...
            <div class="flex justify-end items-center mt-4 border-t pt-3">
                <x-secondary-button
                    x-on:click="$dispatch('close-modal', 'uploadTaskModal{{ $task->id }}')">{{ __('Batal') }}</x-secondary-button>
