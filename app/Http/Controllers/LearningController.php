...
class LearningController extends Controller
{
    ...
    public function showCourse(Classroom $classroom, Course $course)
    {
        ...
        $tasks = Task::where('course_id', $course->id)->paginate(10);
        $taskIds = $tasks->pluck('id')->toArray();
        $taskStudent = TaskStudent::where('user_id', $user->id)
            ->whereIn('task_id', $taskIds)
            ->get();
        $taskStudents = TaskStudent::where('task_id', $taskIds)->paginate(10);

        return view('learning.course', compact('classroom', 'course', 'subjects', 'tasks', 'taskStudent', 'taskStudents'));
    }
